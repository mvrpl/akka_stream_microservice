...
object HttpToGrpc {
  def main(args: Array[String]): Unit = {
    ...
    val settings = GrpcClientSettings.fromConfig("api.StreamService")
    val client = StreamAPIClient(settings)
    ...
    val bindingFuture = Http().newServerAt("127.0.0.1", 8081).bindFlow(route)
